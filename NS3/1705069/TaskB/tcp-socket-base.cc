  ...
  static TypeId tid =
      TypeId("ns3::TcpSocketBase")
          ...
          .AddAttribute("MaxSegLifetime",
                        ...
                        "TIME_WAIT state transition to CLOSED state",
...
uint32_t TcpSocketBase::GetInitialSSThresh(void) const {
...
uint32_t TcpSocketBase::GetInitialCwnd(void) const {
...
void TcpSocketBase::EnterRecovery(uint32_t currentDelivered) {
  ...
  // Modification - Start
  m_tcb->v_count /= 2;
  m_tcb->mode = TcpSocketState::MODERATE;
  // std::cout << "Fast Retransmit, changing mode to moderate" << std::endl;
  // Modification - End
...
void TcpSocketBase::DupAck(uint32_t currentDelivered) {
  ...
  // consider dupacks. Without SACK, there are some euristics in the RFC 6582,
...
void TcpSocketBase::ReceivedAck(Ptr<Packet> packet,
  ...
  uint32_t currentDelivered = static_cast<uint32_t>(
...
uint32_t TcpSocketBase::Window(void) const {
  ...
  if (!m_history.empty()) {
    ...
    if (!h.retx && ackSeq >= (h.seq + SequenceNumber32(
      ...
      if (m_timestampEnabled && tcpHeader.HasOption(TcpOption::TS)) {
        ...
        if (m.IsZero()) {
          NS_LOG_LOGIC("TcpSocketBase::EstimateRtt - RTT calculated from "
                       "TcpOption::TS is zero, approximating to 1us.");
          m = MicroSeconds(1);
        }
      } else {
...
void TcpSocketBase::ReTxTimeout() {
  ...
  m_tcb->v_count = 0;
  m_tcb->mode = TcpSocketState::MODERATE;
  // std::cout << "ReTxTimeout, changing mode to moderate" << std::endl;
...
uint32_t TcpSocketBase::GetSegSize(void) const { return m_tcb->m_segmentSize; }
...
uint32_t TcpSocketBase::GetSynRetries(void) const { return m_synRetries; }
...
uint32_t TcpSocketBase::GetDataRetries(void) const {
  NS_LOG_FUNCTION(this);
...
void TcpSocketBase::AddOptionSack(TcpHeader &header) {
  ...
  for (i = sackList.begin(); allowedSackBlocks > 0 && i != sackList.end();
       ++i) {
...
Time TcpSocketBase::GetMinRto(void) const { return m_minRto; }
...
Ptr<TcpTxBuffer> TcpSocketBase::GetTxBuffer(void) const { return m_txBuffer; }

Ptr<TcpRxBuffer> TcpSocketBase::GetRxBuffer(void) const {
...
bool TcpSocketBase::IsPacingEnabled(void) const {
  if (!m_tcb->m_pacing) {
    return false;
  } else {
...
void TcpSocketBase::UpdatePacingRate(void) {
...
uint32_t TcpSocketBase::GetRWnd(void) const { return m_rWnd.Get(); }

SequenceNumber32 TcpSocketBase::GetHighRxAck(void) const {
