...
int main(int argc, char *argv[]) {
  ...
  std::string prefix_file_name = "TaskB";
  ...
  ApplicationContainer sinkApp = sinkHelper.Install(wifiStaNodes2.Get(0));

  Address sinkAddress(InetSocketAddress(staInterface2.GetAddress(0), sinkPort));
  ...
  for (int i = 0; i < NUM_FLOWS; i++) {
    ...
    Ptr<OutputStreamWrapper> stream =
        asciiTraceHelper.CreateFileStream("taskB_1_" + nodeId.str() + ".cwnd");
    tcpSocket->TraceConnectWithoutContext(
        "CongestionWindow", MakeBoundCallback(&CwndChange, stream));

    PcapHelper pcapHelper;
    Ptr<PcapFileWrapper> file =
        pcapHelper.CreateFile("taskB_1_" + nodeId.str() + ".pcap",
  ...
  fairNess = (1.0 * AvgThroughput * AvgThroughput) / (1.0 * j * sqTp);
