...
TypeId TcpVegas::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::TcpVegas")
...
TcpVegas::~TcpVegas(void) { NS_LOG_FUNCTION(this); }

Ptr<TcpCongestionOps> TcpVegas::Fork(void) {
...
void TcpVegas::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked,
  ...
  // Modification
  if (rtt > tcb->lastRtt && tcb->lastCwnd == tcb->m_cWnd) {
    tcb->v_count++;

    if (tcb->v_count > tcb->v_count_max) {
      tcb->mode = tcb->AGGRESIVE;
    }
  } else if (rtt < tcb->lastRtt) {
    tcb->v_count = std::max(tcb->v_count - 1, 0);
  }

  std::cout << "v_count=" << tcb->v_count << ", last_rtt=" << tcb->lastRtt
            << ", vegasMode=" << tcb->mode << std::endl;

  tcb->lastRtt = rtt;
  tcb->lastCwnd = tcb->m_cWnd;
}
...
void TcpVegas::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked) {
  ...
  if (!m_doingVegasNow || tcb->mode == TcpSocketState::AGGRESIVE) {
